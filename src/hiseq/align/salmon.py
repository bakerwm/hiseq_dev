...
Basic usage: 
salmon quant -i $index -l A -p 8 --gcBias -o ${out_dir}/$prefix -1 $r1 -2 $r2 
# output
# quant.sf
tximport()
txmeta()
...
from hiseq.utils.file import check_file, check_dir, file_abspath, file_exists
...
class SalmonConfig(object):
    ...
    def init_fx(self):
        ...
        if not check_fx_args(self.fq1, self.fq2):
            raise ValueError("--fq1, --fq2 failed, not properly paired")
    ...
    def init_files(self):
        ...
        default_files = {
            ...
            "align_log": os.path.join(
                self.project_dir, "logs", "salmon_quant.log"
            ),
...
class Salmon(object):
    ...
    def get_cmd(self):
        """
        Example:
        salmon quant -i $index --gcBias -l A -p 8 -o ${out_dir}/$prefix -1 $r1 -2 $r2
        """
        # fq
        cmd_fx = (
            "-1 {} -2 {}".format(self.fq1, self.fq2)
            if self.fq2
            else "-r {}".format(self.fq1)
        )
        self.cmd = " ".join(
            [
    ...
    def run(self):
        ...
        df.update({"name": self.smp_name, "index": self.index_name})
        Config().dump(df, self.align_json)
        if not file_exists(self.quant_sf):
            log.error("salmon() failed, no output: {}".format(self.quant_sf))
...
def get_args():
    ...
    parser.add_argument(
        "-x", "--index", required=True, help="The alignment index for salmon"
    )
    parser.add_argument(
        "-in",
        "--index-name",
    ...
    parser.add_argument(
        "-p", "--threads", default=1, help="Number of threads, default: [1]"
    )
    parser.add_argument(
        "-w",
        "--overwrite",
