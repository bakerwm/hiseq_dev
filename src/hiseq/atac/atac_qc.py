...
from hiseq.utils.file import (
    check_file, check_dir, copy_file, copy_dir, file_exists, file_nrows,
)
...
def qc_trim_summary(x, hiseq_type='r1'):
    if not is_hiseq_dir(x, hiseq_type):
        log.error('qc_trim_summary() skipped, not a {} dir: {}'.format(hiseq_type, x))
        return None
    ...
    if a.is_hiseq_r1:
        ...
        if isinstance(r1_list, list) and len(r1_list) > 1:
            for i in r1_list:
                di = Config().load(i)
                d1 = {k:d.get(k, 0)+di.get(k, 0) for k in ['input', 'output']}
                d.update(d1)
            # update name, pct
            d.update({
                'name': a.smp_name,
                'out_pct': round(d.get('output', 0)/d.get('input', 1)*100, 2),
            })
...
def qc_align_summary(x, hiseq_type='auto'):
    ...
    """
    if not is_hiseq_dir(x, hiseq_type):
        log.error('qc_align_summary() skipped, not a {} dir: {}'.format(hiseq_type, x))
        return None
    a = read_hiseq(x, hiseq_type) # for general usage
    if a.is_hiseq_r1:
        # spikein, rRNA, genome
