...
def hiseq_trim(x, hiseq_type="_r1"):
    ...
    if not is_hiseq_dir(x, "_r1"):
        log.error(
            "hiseq_atac_trim() skipped, not a {} dir: {}".format(hiseq_type, x)
        )
        return None
...
def hiseq_align_spikein(x, hiseq_type="_r1"):
    ...
    if not is_hiseq_dir(x, "_r1"):
        log.error(
            "hiseq_align_spikein() skipped, not a hiseq_r1 dir: {}".format(x)
        )
        return None
    a = read_hiseq(x, hiseq_type)  # for general usage
    ...
    if file_exists(a.spikein_bam) and not a.overwrite:
        log.info(
            "hiseq_align_spikein() skipped, file exists: {}".format(
                a.spikein_bam
            )
        )
    else:
...
def hiseq_align_genome(x, hiseq_type="_r1"):
    ...
    if not is_hiseq_dir(x, "_r1"):
        log.error(
            "hiseq_align_genome() skipped, not a hiseq_r1 dir: {}".format(x)
        )
        return None
    a = read_hiseq(x, hiseq_type)  # for general usage
    # init index: spikein, spikein_index
    ...
    if a.rm_dup:
        if file_exists(a.bam_raw):
            if check_file(a.bam, check_empty=True) and not a.overwrite:
                log.info("rm_dup() skipped, file exists: {}".format(a.bam))
            else:
...
def hiseq_merge_trim(x, hiseq_type="_rn"):
    ...
    if not is_hiseq_dir(x, "_rn"):
        log.error(
            "hiseq_merge_bam() skipped, not a hiseq_rn dir: {}".format(x)
        )
        return None
    a = read_hiseq(x, hiseq_type)  # for general usage
...
def hiseq_merge_bam(x, hiseq_type="_rn"):
    ...
    if isinstance(bam_list, list) and len(bam_list) > 1:
        ...
        # calculate norm scale
        s = hiseq_norm_scale(
            x, hiseq_type=hiseq_type, by_spikein=False
        )  # to genome
...
def hiseq_copy_r1(x, hiseq_type="_rn"):
    ...
    if isinstance(bam_list, str):
        ...
        r1_qc_files = list_dir(
            r1_qc_dir, include_dirs=True
        )  # update list_hiseq_file
...
def hiseq_norm_scale(x, hiseq_type="_r1", by_spikein=False, norm=1000000):
    ...
    if not is_hiseq_dir(x, hiseq_type):
        log.error(
            "hiseq_norm_scale() skipped, not a {} dir: {}".format(
                hiseq_type, x
            )
        )
        return None
...
def hiseq_pcr_dup(x, hiseq_type="r1"):
    ...
    if not is_hiseq_dir(x, hiseq_type):
        log.error(
            "hiseq_merge_bam() skipped, not a {} dir: {}".format(hiseq_type, x)
        )
        return None
    a = read_hiseq(x, hiseq_type)  # for general usage
    ...
    if all(file_exists([raw, nodup])):
        ...
        d0["dup_pct"] = (
            round(d0["dup"] / d0["total"], 4) if d0["total"] > 0 else 0
        )
...
def get_mito_count(x):
    ...
    try:
        ...
        n_mt = sum(
            map(
                int, [x.split("\t")[2] for x in lines if not x.startswith("#")]
            )
        )
    except IndexError as e:
        msg = "cannot read bam file, msg=%s, data=%s".format((e, lines))
        log.error(msg)
        n_mt = 0
    return n_mt
...
def hiseq_call_peak(x, hiseq_type="_r1"):
    ...
    if not is_hiseq_dir(x, hiseq_type):
        log.error(
            "hiseq_call_peak() skipped, not a {} dir: {}".format(hiseq_type, x)
        )
        return None
    a = read_hiseq(x, hiseq_type)  # for general usage
...
def hiseq_bam2bw(x, hiseq_type="_r1"):
    ...
    if not is_hiseq_dir(x, hiseq_type):
        log.error(
            "hiseq_bam2bw() skipped, not a {} dir: {}".format(hiseq_type, x)
        )
        return None
    a = read_hiseq(x, hiseq_type)  # for general usage
