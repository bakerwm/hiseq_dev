...
from hiseq.bam2bw.bam2bw_args import (
    get_bam_args,
    add_io_parser,
    add_bam_parser,
)
...
class Bam2bw(object):
    ...
    def update_args(self):
        ...
        if not is_valid_file(self.bam_list, is_valid_bam):
            msg = "\n".join(
                [
                    "{} : {}".format(is_valid_file(i, is_valid_bam), i)
...
class Bam2bw_ns(object):
    ...
    def get_effsize(self):
        ...
        if not isinstance(s, int):
            raise ValueError(
                "could not determine effsize: {}".format(self.bam)
            )
        return s
    ...
    def get_cmd(self):
        ...
        dlist = [
            "--{} {}".format(k, v) for k, v in args.items() if v is not None
        ]
    ...
    def run(self):
        ...
        if not is_valid_file(self.bw, is_valid_bigwig):
            log.error(
                "bamCoverage() failed, file not found: {}".format(self.bw)
            )
        return self.bw
...
class Bam2bw_ss(object):
    ...
    def run(self):
        ...
        if not is_valid_file([self.bw_fwd, self.bw_rev], is_valid_bigwig):
            log.error(
                "bamCoverage() failed, {} {}".format(self.bw_fwd, self.bw_rev)
            )
