...
def add_io_parser(parser):
    if not isinstance(parser, argparse.ArgumentParser):
        ...
        return parser
    parser.add_argument(
        "-b", dest="bam_list", nargs="+", required=True, help="bam files"
    )
    parser.add_argument(
        "-o",
        dest="out_dir",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Strand-specific, dUTP library",
    )
...
def add_bam_parser(parser):
    ...
    parser.add_argument(
        "-sm", "--smoothLength", type=int, default=None, help="smooth length"
    )
    parser.add_argument(
        "-fs",
        "--filterRNAstrand",
