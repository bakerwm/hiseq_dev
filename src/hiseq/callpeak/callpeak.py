...
class CallPeak(object):
    ...
    def init_args(self):
        ...
        if not isinstance(self.out_dir, str):
            ...
            log.warning("set pwd as out_dir: {}".format(self.out_dir))
    ...
    def init_genome(self):
        ...
        if isinstance(self.genome, str):
            ...
            if self.genome_size < 1:
                raise ValueError(
                    "genome_size, failed, {}".format(self.genome_size)
                )
            elif not isinstance(self.genome_size_file, str):
                raise ValueError(
                    "genome_size_file, failed, {}".format(
    ...
    def init_files(self):
        default_files = {
            "config_toml": self.out_dir + "/config.toml",
            "macs2_peak": self.out_dir
            + "/"
            + self.prefix
            + "_peaks.narrowPeak",
            "macs2_stdout": self.out_dir + "/" + self.prefix + ".macs2.stdout",
            "macs2_stderr": self.out_dir + "/" + self.prefix + ".macs2.stderr",
            ...
            "seacr_peak_top": self.out_dir
            + "/"
            + self.prefix_top001
            + ".stringent.bed",
            "seacr_stdout": self.out_dir + "/" + self.prefix + ".seacr.stdout",
            "seacr_stderr": self.out_dir + "/" + self.prefix + ".seacr.stderr",
        }
    ...
    def run_macs2(self):
        ...
        if file_exists(self.macs2_peak) and not self.overwrite:
            log.info(
                "run_macs2() skipped, file exists:{}".format(self.macs2_peak)
            )
        else:
            try:
                run_shell_cmd(cmd)
                ...
                log.error(
                    "run_macs2() failed, check {}".format(self.macs2_log)
                )
    ...
    def run_seacr(self):
        ...
        if file_exists(self.ip):
            cmd1 = " ".join(
                [
                    ...
                    "non stringent {}".format(
                        self.out_dir + "/" + self.prefix_top001
                    ),
            ...
            # save cmd
            cmd1_txt = self.out_dir + "/" + self.prefix + ".SEACR.top0.01.sh"
            with open(cmd1_txt, "wt") as w:
                w.write(cmd1 + "\n")
            if file_exists(self.seacr_peak_top) and not self.overwrite:
                log.info(
                    "run_seacr() skipped, file exists:{}".format(
                        self.seacr_peak_top
                    )
                )
            else:
        ...
        if file_exists(self.input):
            ...
            # save cmd
            cmd2_txt = (
                self.out_dir + "/" + self.prefix + ".SEACR.ip_vs_input.sh"
            )
            ...
            if file_exists(self.seacr_peak) and not self.overwrite:
                ...
                try:
                    ...
                    log.error(
                        "run_seacr() failed, check {}".format(self.seacr_peak)
                    )
    ...
    def run(self):
        ...
        if self.method.lower() == "macs2":
            ...
            raise ValueError(
                "unknown method: {macs2|seacr}, got {}".format(self.method)
            )
        # remove files
        del_list = []
