...
from hiseq.utils.file import check_dir, file_abspath, list_dir
...
class CnrMerge(object):
    ...
    def run(self):
        ...
        CnrRp(self.out_dir).run() # generate report
...
class CnrMergeConfig(object):
    ...
    def init_args(self):
        ...
        if self.out_dir is None:
            self.out_dir = os.path.join(str(pathlib.Path.cwd()), 'hiseq_merge')
        ...
        if isinstance(x, str):
            if os.path.isdir(x):
                ...
                if is_hiseq_dir(x, hiseq_type):
                    out.append(x)
                # level-2
                d = [i for i in list_dir(x, include_dirs=True) if is_hiseq_dir(i, hiseq_type)]
    ...
    def init_files(self):
        ...
        check_dir(dir_list, create_dirs=True)
