...
def cnr_align_spikein(x, hiseq_type="_r1"):
    ...
    if not a.is_hiseq:
        log.error(
            "cnr_align_spikein() skipped, not a cnr_r1 dir: {}".format(x)
        )
        return None
    ...
    if file_exists(a.spikein_bam) and not a.overwrite:
        log.info(
            "cnr_align_spikein() skipped, file exists: {}".format(
                a.spikein_bam
            )
        )
    else:
...
def cnr_align_genome(x, hiseq_type="_r1"):
    ...
    align_extra = (
        "-I 10 -X 2000"
        if a.hiseq_type.startswith("atac")
        else "-I 10 -X 700"
        if a.hiseq_type.startswith("cn")
        else ""
    )  # cnr, cnt
    args_init = {
        "aligner": a.aligner,
...
def cnr_merge_bam(x, hiseq_type="_rn"):
    ...
    if isinstance(bam_list, list) and len(bam_list) > 1:
        ...
        if not file_exists(a.bam):
            raise ValueError(
                "cnr_merge_bam() failed, see: {}".format(a.bam_dir)
            )
        # save align_json
        d = {"name": a.smp_name}
...
def hiseq_pcr_dup(x, hiseq_type="r1"):
    ...
    if not a.is_hiseq:
        log.error(
            "qc_dup_summary() skipped, not a rnaseq_r1 dir: {}".format(x)
        )
        return None
    # name total nodup dup
    out = None
...
def cnr_bw_compare(x, hiseq_type="rx"):
    ...
    if not (a.is_hiseq and hiseq_type.endswith("rx")):
        ...
        return None
    bw_compare(
        a.ip_bw, a.input_bw, a.bw, "subtract", threads=a.threads, binsize=100
    )
...
def qc_align_summary(x, hiseq_type="auto"):
    ...
    if not a.is_hiseq:
        log.error(
            "qc_align_summary() skipped, not a rnaseq_r1 dir: {}".format(x)
        )
        return None
    if a.is_hiseq_r1:
        # spikein, rRNA, genome
