...
class CnRn(object):
        ...
        if rm_dup:
            if file_exists(self.bam):
                if file_exists(self.bam_rmdup) and not self.overwrite:
                    ...
                    Bam(self.bam).rm_dup(self.bam_rmdup)
    ...
    def run_fq_single(self, i):
        ...
        args_required = ['aligner', 'fq1', 'fq2', 'genome', 'gene_bed',
            'genome_size', 'is_ip', 'trimmed', 'out_dir', 'overwrite', 
...
class CnR1(object):
    ...
    align - bam - rm_dup - proper_paired - peak - bw - qc - report
    """
    def __init__(self, **kwargs):
        self = update_obj(self, kwargs, force=True)
        self.init_args()
    ...
    def trim(self, trimmed=False):
        ...
        Trim reads:
        Trimmer(fq1, out_dir, fq2, cut_after_trim='9,-6').run()
...
class CnRxConfig(object):
    ...
    def init_fq(self):
        ...
        if isinstance(self.ip, list) and isinstance(self.input, list):
            ...
            self.ip_dir = self.out_dir + '/' + self.ip_name
            self.input_dir = self.out_dir + '/' + self.input_name
    ...
    def init_files(self, create_dirs=True):
        ...
        self.project_dir = self.out_dir + '/' + self.project_name
        self.config_dir = self.project_dir + '/config'
...
class CnRnConfig(object):
    ...
    def init_fq(self):
        ...
        if isinstance(self.rep_list, list):
            ...
            self.rep_list = [self.out_dir + '/' + i for i in q_names]
...
class Align(object):
    ...
    def init_args(self):
        ...
        if not isinstance(self.out_dir, str):
            self.out_dir = str(pathlib.Path.cwd())
            log.warning('set pwd as out_dir: {}'.format(self.outdir))
        self.out_dir = file_abspath(self.outdir)
    ...
    def init_files(self):
        ...
        """
        # subdir
        subdir = os.path.join(self.out_dir, self.smp_name, self.index_name)
        check_dir(subdir)
        ...
        default_files = {
            ...
            'rm_dup_bam': prefix + '.rmdup.bam',
            'rm_dup_matrix': prefix + '.rmdup.matrix.txt'
        }
...
class CallPeak(object):
    ...
    def init_files(self):
        # files
        default_files = {
            'config_pickle': self.out_dir + '/config.pickle',
            'macs2_peak': self.out_dir + '/' + self.prefix + '_peaks.narrowPeak',
            'macs2_log': self.out_dir + '/' + self.prefix + '.callpeak.log',
            'macs2_peak_xls': self.out_dir + '/' + self.prefix + '_peaks.xls',
            'ip_bed': self.out_dir + '/' + self.ip_name + '.frag.bed',
            'ip_bg': self.out_dir + '/' + self.ip_name + '.frag.bg',
            'input_bed': self.out_dir + '/' + self.input_name + '.frag.bed',
            'input_bg': self.out_dir + '/' + self.input_name + '.frag.bg',
            'seacr_peak': self.out_dir + '/' + self.prefix + '.stringent.bed',
            'seacr_peak_top': self.out_dir + '/' + self.prefix_top001 + '.stringent.bed'
        }
        self = update_obj(self, default_files, force=True) # key
    ...
    def run_macs2(self):
        ...
        cmd = ' '.join([
            ...
            '-n {}'.format(self.out_dir + '/' + self.prefix),
            '-q 0.1 --keep-dup all',
            '--nomodel --extsize 150',
        ...
        # save cmd
        cmd_txt = self.out_dir + '/' + self.prefix + '.macs2.cmd.sh'
        with open(cmd_txt, 'wt') as w:
            w.write(cmd + '\n')
    ...
    def bampe_to_bg(self, bam, bg):
        ...
        # save cmd
        cmd_txt = self.out_dir + '/' + self.prefix + '.bam2bg.cmd.sh'
        with open(cmd_txt, 'wt') as w:
            w.write(cmd + '\n')
    ...
    def run_seacr(self):
        ...
        cmd = ' '.join([
            ...
            'non stringent {}'.format(self.out_dir + '/' + self.prefix_top001)
        ])
        ...
        if not self.input is None:
            ...
            cmd = ' '.join([
                ...
                'non stringent {}'.format(self.out_dir + '/' + self.prefix)            
            ])

        # save cmd
        cmd_txt = self.out_dir + '/' + self.prefix + '.SEACR.cmd.sh'
        with open(cmd_txt, 'wt') as w:
            w.write(cmd + '\n')
