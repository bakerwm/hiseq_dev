...
class Demx2(object):
    ...
    def init_args(self):
        ...
        print(
            f"Detected [{self.n_fq}] fastq (pairs), and [{self.n_idx}] index from sample sheet"
        )
    ...
    def load_fq(self):
        ...
        if len(self.fq_list) > 1:  # paired-end mode only
            ...
            if min(fq_ver_list) == 0:
                f_list = [
                    self.fq_list[i]
                    for i, j in enumerate(fq_ver_list)
                    if j == 0
                ]
    ...
    def load_index(self):
        ...
        if any(dup1):
            ...
            raise ValueError(
                f"Duplicate i7/i5/barcode found: {self.sample_sheet}"
            )
    ...
    def fq_paired(self, x):
        ...
        if isinstance(x, str):
            ...
            p2 = re.compile(
                "(\\.|_)(R?[12])\\.(f(ast)?q+)(\\.gz)?$", flags=re.IGNORECASE
            )
            if p1.search(x):
                if p2.search(x):
                    # paired-end
    ...
    def stat_fq(self):
        ...
        for k, v in self.fq_info.items():  # see self.rename_fq()
            ...
            print(
                f"[{i}/{self.n_fq}] - {Path(fq1).name}", end="\r"
            )  # progress
            # check reads
    ...
    def report(self):
        ...
        fq_count = sum(
            [v.get("count", 0) for k, v in self.fq_metadata.items()]
        )
