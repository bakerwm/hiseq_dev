...
class DemxR1(object):
    ...
    def init_args(self):
        ...
        if not self.mismatch in range(4):
            raise ValueError(
                "illegal mismatch: [{}], expect [0,1,2,3]".format(
                    self.mismatch
                )
            )
        # choose demx function
        self.guess_name = (
            self.guess_index_name
            if self.index_type == "i7"
            else self.guess_barcode_name
        )
        # index table
    ...
    def load_index(self, x, min_cols=2, parse_read_count=False):
        ...
        try:
            with open(x) as r:
                for l in r:
                    ...
                    if i7 in d:
                        raise ValueError(
                            "index not unique, {}".format(self.index_table)
                        )
                    # parse read count
        ...
        if not self.is_valid_index(list(d.keys())):
            raise ValueError(
                "index not compatible with mismatch={}".format(self.mismatch)
            )
        # load barcode width
    ...
    def guess_barcode_name(self, x):
        ...
        s = x[1]  # seq
        query = s[
            self.barcode_n_left : (self.barcode_n_left + self.barcode_width)
        ]
        # search index name
        hit = None
        for i, j in self.idx.items():
    ...
    def demx_se(self, fh):
        ...
        with ExitStack() as stack:
            ...
            for r1 in readfq(fh):
                ...
                if r1[-1] is None:
                    raise ValueError(
                        "comment field missing: {}".format(self.fq1)
                    )
    ...
    def demx_pe(self, fh1, fh2):
        ...
        sx = ".fq.gz" if self.gzipped else ".fq"
        fq1_files = [
            os.path.join(self.out_dir, i + "_1" + sx) for i in fq_names
        ]
        fq2_files = [
            os.path.join(self.out_dir, i + "_2" + sx) for i in fq_names
        ]
        fn = {}
        with ExitStack() as stack:
            ...
            for r1, r2 in zip(readfq(fh1), readfq(fh2)):
                ...
                if not r1[0] == r2[0]:
                    raise ValueError(
                        "file not paired, check: {}".format(r1[0])
                    )
                # update name
    ...
    def show_msg(self):
        msg = "\n".join(
            [
                ...
                "{:>20} : {}".format(
                    "PE_mode", "yes" if self.is_paired else "no"
                ),
    ...
    def report(self):
        ...
        df2 = (
            Config().load(self.fn_json) if file_exists(self.fn_json) else None
        )
        if df2 is None:
            df2 = {}
...
def get_args():
    ...
    parser.add_argument(
        "-1", "--fq1", required=True, help="read1 in fastq format, gzipped"
    )
    parser.add_argument(
        "-2", "--fq2", help="read2 in fastq format, gzipped, (optional)"
    )
    parser.add_argument(
        "-s",
        "--index-table",
