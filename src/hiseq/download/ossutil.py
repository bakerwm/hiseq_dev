...
class TOS(object):
    ...
    def open_client(self):
        try:
            client = tos.TosClientV2(self.accesskeyid,
    ...
    def ls(self):
        ...
        if isinstance(content_list, list):
            ...
            if len(key_list) > 0:
                print(f'{key_list[0]}\n...')
                print(f'[{len(key_list)}] files [{total_size:.1f}] GB in total')
            return key_list
    ...
    def download_key(self, key):
        try:
            if isinstance(key, str):
                ...
                local_file = str(Path(self.out_dir) /
                ...
                if Path(local_file).exists() and not self.overwrite:
                    ...
                    self.client.download_file(
                        self.bucket_name, key, local_file, 
                        task_num=3, 
    ...
    def utils(self, name='percentage'):
        ...
        # event
        def download_event(type: DownloadEventType, err, bucket,
                           key, version_id, file_path, checkpint_file,
                           tmp_file, download_part):
            print(type, err, bucket, key, version_id, file_path,
                  checkpint_file, tmp_file, download_part,
                  end='\r')

        # set speed limitation, eg: 10 KB/s, max 10 + 5 KB/s
        rate_limiter = tos.RateLimiter(rate=self.maxspeed * 1024 * 8,
                                       capacity=5 * 1024 * 8)
            ...
            if isinstance(key, str):
                ...
                if Path(local_file).exists() and not self.overwrite:
                    ...
                    oss2.resumable_download(
                        ...
                        multiget_threshold=1 * 1024 * 1024, # threshold for multiget, 1 MB
                        part_size=1 * 1024 * 1024, # 1 MB [100 KB - 5 GB]
                        progress_callback=self.utils('percentage'),
                        num_threads=3, headers=headers
                    )
                return local_file
            else:
                print(f'download(key) expect str, got {type(key)}')
