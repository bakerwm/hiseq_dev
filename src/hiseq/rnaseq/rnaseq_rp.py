...
class RnaseqRp(object):
    ...
    def init_args(self):
        ...
        if not file_exists(self.project_dir):
            raise ValueError(
                "project_dir not valid: {}".format(self.project_dir)
            )
        # check hiseq_dir
        a = read_hiseq(self.project_dir)
        if not a.is_hiseq:
            raise ValueError(
                "project_dir not hiseq_dir: {}".format(self.project_dir)
            )
        # check default files
    ...
    def report(self):
        ...
        rnaseq_report_html = os.path.join(self.report_dir, "HiSeq_report.html")
        cmd = " ".join(
            [
        ...
        if file_exists(rnaseq_report_html) and not self.overwrite:
            log.info(
                "report() skipped, file exists: {}".format(rnaseq_report_html)
            )
        else:
            run_shell_cmd(cmd)
