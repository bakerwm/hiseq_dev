...
class RnaseqRx(object):
    ...
    def run_rn(self):
        ...
        args_global = dict(
            (k, getattr(self, k)) for k in args_required if k in self.__dict__
        )
        # for mut
    ...
    def copy_r1_files(self):
        ...
        for i in [self.mut_dir, self.wt_dir]:
            for j in ["r1", "rn"]:
                ...
                [
                    symlink_file(bam + ".bai", self.bam_dir) for bam in b
                ]  # add bai
...
class RnaseqRxConfig(object):
    ...
    def init_files(self):
        ...
        default_files = {
            ...
            # qc files
            "genebody_enrich_matrix": os.path.join(
                self.qc_dir, "05.genebody_enrich.mat.gz"
            ),
            "genebody_enrich_matrix_log": os.path.join(
                self.qc_dir, "05.genebody_enrich.log"
            ),
            "genebody_enrich_png": os.path.join(
                self.qc_dir, "05.genebody_enrich.png"
            ),
            "genebody_enrich_cmd": os.path.join(
                self.qc_dir, "05.genebody_enrich.cmd.sh"
            ),
            "bam_cor_npz": os.path.join(self.qc_dir, "06.bam_cor.npz"),
            "bam_cor_counts": os.path.join(
                self.qc_dir, "06.bam_cor.counts.tab"
            ),
            "bam_cor_heatmap_png": os.path.join(
                self.qc_dir, "06.bam_cor.cor_heatmap.png"
            ),
            "bam_cor_pca_png": os.path.join(
                self.qc_dir, "06.bam_cor.cor_PCA.png"
            ),
            # deseq
            "deseq_fix_xls": os.path.join(
                self.deseq_dir, "transcripts_deseq2.fix.xls"
            ),
        }
...
def get_args():
    ...
    parser.add_argument(
        "--bed", dest="gene_bed", default=None, help="The BED of genes"
    )
