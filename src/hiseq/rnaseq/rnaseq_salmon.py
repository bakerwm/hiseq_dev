...
class RnaseqSalmonPipe(object):
    ...
    def run_single_rx(self, i):
        i = {
            k: v
            for k, v in i.items()
            if k in ["mut_fq1", "mut_fq2", "wt_fq1", "wt_fq2"]
        }
        # update outdir
    ...
    def run(self):
        if self.build_design:
            ...
            if AlignIndex(self.salmon_index).is_valid():
                ...
                log.error(
                    "salmon_index not valid: {}".format(self.salmon_index)
                )
...
class RnaseqSalmonConfig(object):
    ...
    def init_fq(self, fq1, fq2):
        ...
        if not isinstance(fq1, list):
            raise ValueError(
                "fq1 require list, got {}".format(type(fq1).__name__)
            )
    ...
    def init_index(self):
        ...
        if not AlignIndex(self.salmon_index, aligner="salmon").is_valid():
            raise ValueError(
                "--salmon-index, not valid: {}".format(self.salmon_index)
            )
        # index_name #updated
    ...
    def init_files(self):
        ...
        # update rep dirs
        self.mut_dirs = [
            os.path.join(self.quant_dir, i)
            for i in fx_name(self.mut_fq1, fix_pe=self.is_paired)
        ]
        self.wt_dirs = [
            os.path.join(self.quant_dir, i)
            for i in fx_name(self.wt_fq1, fix_pe=self.is_paired)
        ]
        # quant.sf files
    ...
    def init_quant_files(self):
        ...
        for fq1 in self.mut_fq1:
            ...
            sf = os.path.join(
                self.quant_dir, smp_name, self.index_name, "quant.sf"
            )
