...
class RNAseqRx(object):
    ...
    def run_RNAseqRn(self):
        ...
        if c_mutant.is_hiseq_rn and c_wildtype.is_hiseq_rn:
            ...
            wildtype_args.update(
                {"fq1": self.wildtype, "fq2": self.wildtype_fq2}
            )
            ...
            mutant_args.update({"fq1": self.mutant, "fq2": self.mutant_fq2})
    ...
    def run_deseq(self):
        ...
        cmd = "Rscript {} {} {} 1> {} 2> {}".format(
            deseqR, self.project_dir, run_go, deseq_stdout, deseq_stderr
        )
        with open(cmd_shell, "wt") as w:
            w.write(cmd + "\n")
        if file_exists(self.deseq_fix_xls) and not self.overwrite:
            log.info(
                "run_deseq() skipped, file exists: {}".format(
                    self.deseq_fix_xls
                )
            )
        else:
...
class RNAseqRn(object):
    ...
    def init_args(self):
        ...
        # update genome_size:
        if self.extra_index:
            self.genome_size = AlignIndex(
                index=self.extra_index, aligner=self.aligner
            ).index_size()
    ...
    def merge_bam(self):
        ...
        if os.path.exists(self.bam):
            log.info("merge_bam() skipped, file exists: {}".format(self.bam))
        else:
            try:
                run_shell_cmd(cmd)
            except:
    ...
    def get_norm_scale(self):
        ...
        try:
            ...
            log.warning(
                "could not read scale from project: {}".format(
                    self.norm_project
                )
            )
            log.warning(
                "force norm by current project: {}".format(self.project_dir)
            )
            out = None
        if not isinstance(out, float):
            try:
                ...
                log.error(
                    "force scale=1, failed: {},".format(self.align_scale_txt)
                )
                out = 1
            # save to txt
...
class RNAseqR1(object):
    ...
    def align_genome(self):
        ...
        if args_local.get("genome_index", None) == args_local.get(
            "extra_index", None
        ):
    ...
    def fc_count(self):
        ...
        msg = "\n".join(
            ["sens: {}".format(strand_sens), "anti: {}".format(strand_anti)]
        )
    ...
    def pipe_genome(self):
        ...
        if file_exists(self.count_sens) and not self.overwrite:
            log.info(
                "fc_count() skipped, file exists: {}".format(self.count_sens)
            )
        else:
            self.fc_count()
...
class RNAseqRp(object):
    ...
    def report(self):
        ...
        if file_exists(self.report_html):
            log.info(
                "report() skipped, file exists: {}".format(self.report_html)
            )
        else:
            try:
                run_shell_cmd(cmd)
...
class RNAseqConfig(object):
    ...
    def init_args(self):
        ...
        self.threads, self.parallel_jobs = init_cpu(
            self.threads, self.parallel_jobs
        )

        # RNAseq groups
        self.fq_groups = (
            Config().from_toml(self.design)
            if file_exists(self.design)
            else self.init_groups()
        )
    ...
    def init_groups(self):
        ...
        if isinstance(self.wildtype, list) and isinstance(self.mutant, list):
        ...
        elif isinstance(self.wildtype_dir, str) and isinstance(
            self.mutant_dir, str
        ):
            c_wildtype = RNAseqReader(self.wildtype_dir)
            c_mutant = RNAseqReader(self.mutant_dir)
            ...
            if not c_wildtype.is_hiseq_rn or not c_mutant.is_hiseq_rn:
                raise ValueError(
                    "mutant_dir, wildtype_dir failed: \
                    {}, {}".format(
                        self.mutant_dir, self.wildtype_dir
                    )
                )
            g = {
...
class RNAseqRxConfig(object):
    ...
    def init_fq(self):
        ...
        if isinstance(self.mutant, list) and isinstance(self.wildtype, list):
            ...
            if not all(file_exists(self.mutant)):
                raise ValueError(
                    "--mutant, file not exists: {}".format(self.mutant)
                )

            if not all(file_exists(self.wildtype)):
                raise ValueError(
                    "--wildtype, file not exists: {}".format(self.mutant)
                )
            ...
            if isinstance(self.mutant_fq2, list):
                mutant_pe = all(
                    [
                        fq_paired(i, j)
                        for i, j in zip(self.mutant, self.mutant_fq2)
                    ]
                )
                if not mutant_pe:
                    raise ValueError(
                        "--mutant, --mutant-fq2, not paired: \
                        {}, {}".format(
                            self.mutant, self.mutant_fq2
                        )
                    )
            if isinstance(self.wildtype_fq2, list):
                wildtype_pe = all(
                    [
                        fq_paired(i, j)
                        for i, j in zip(self.wildtype, self.wildtype_fq2)
                    ]
                )
                if not wildtype_pe:
                    raise ValueError(
                        "--wildtype, --wildtype-fq2, not paired: \
                        {}, {}".format(
                            self.wildtype, self.wildtype_fq2
                        )
                    )
        ...
        elif isinstance(self.mutant_dir, str) and isinstance(
            self.wildtype_dir, str
        ):
    ...
    def init_files(self):
        ...
        default_files = {
            ...
            "mutant_bw_fwd": self.bw_dir
            + "/"
            + self.mutant_name
            + ".fwd.bigWig",
            "mutant_bw_ref": self.bw_dir
            + "/"
            + self.mutant_name
            + ".rev.bigWig",
            "wildtype_bam": self.bam_dir + "/" + self.wildtype_name + ".bam",
            "wildtype_bw": self.bw_dir + "/" + self.wildtype_name + ".bigWig",
            "wildtype_bw_fwd": self.bw_dir
            + "/"
            + self.wildtype_name
            + ".fwd.bigWig",
            "wildtype_bw_rev": self.bw_dir
            + "/"
            + self.wildtype_name
            + ".rev.bigWig",
    ...
    def init_fq(self):
        ...
        if isinstance(self.fq1, list):
            ...
            if isinstance(self.fq2, list):
                ...
                # paired
                fq_pe = all(
                    [fq_paired(i, j) for i, j in zip(self.fq1, self.fq2)]
                )
...
class RNAseqR1Config(object):
    ...
    def init_index(self):
        ...
        if isinstance(self.rRNA_index, str):
            ...
            if not ai.is_index():
                raise ValueError(
                    "rRNA_index failed, {}".format(self.rRNA_index)
                )
            ...
            if isinstance(self.genome, str):
                ...
                for rRNA in ["rRNA", "MT_trRNA", "trRNA"]:
                    ...
                    if ri_index:
                        log.info(
                            "fetch rRNA index: {}, {}".format(rRNA, ri_index)
                        )
        ...
        if isinstance(self.spikein_index, str):
            ...
            if not ai.is_index():
                raise ValueError(
                    "spikein_index failed, {}".format(self.spikein_index)
                )
...
class RNAseqRpConfig(object):
    ...
    def init_args(self):
        ...
        if self.hiseq_type == None:
            raise Exception(
                "project_dir, not a hiseq_ dir: {}".format(self.project_dir)
            )
...
class HiSeqLibType(object):
    ...
    def init_args(self):
        ...
        if not isinstance(self.bam, str):
            raise Exception(
                "bam, str and list expected, got {}".format(
        ...
        if not isinstance(self.gtf, str):
            raise Exception(
                "gtf, str expected, got {}".format(type(self.gtf).__name__)
            )
