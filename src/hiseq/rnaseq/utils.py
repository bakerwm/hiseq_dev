...
def rnaseq_align_spikein(x, hiseq_type="rnaseq_r1"):
    ...
    **Update clean data**
    ...
    if not a.is_hiseq:
        log.error(
            "rnaseq_align_spikein() skipped, not a rnaseq_r1 dir: {}".format(x)
        )
        return None
    ...
    if file_exists(a.spikein_bam) and not a.overwrite:
        log.info(
            "rnaseq_align_spikein() skipped, file exists: {}".format(
                a.spikein_bam
            )
        )
    else:
...
def rnaseq_align_rRNA(x, hiseq_type="r1"):
    ...
    if not a.is_hiseq:
        log.error(
            "rnaseq_align_rRNA() skipped, not a rnaseq_r1 dir: {}".format(x)
        )
        return None
...
def rnaseq_align_genome(x, hiseq_type="r1"):
    ...
    if not a.is_hiseq:
        log.error(
            "rnaseq_align_genome() skipped, not a rnaseq_r1 dir: {}".format(x)
        )
        return None
    args_local = a.__dict__
    ##################################################################
...
def rnaseq_quant(x, hiseq_type="r1"):
    ...
    if not all(file_exists([gtf, bam])):
        msg = (
            "rnaseq_quant() skipped, file not exists, bam: {}, gtf: {}".format(
...
def rnaseq_deseq(x, hiseq_type="rx"):
    ...
    if file_exists(a.deseq_fix_xls) and not a.overwrite:
        log.info(
            "rnaseq_deseq() skipped, file exists: {}".format(a.deseq_fix_xls)
        )
    else:
        try:
            run_shell_cmd(cmd)
...
def deseq_salmon(x, hiseq_type="rx"):
    ...
    if file_exists(a.deseq_fix_xls) and not a.overwrite:
        log.info(
            "deseq_salmon() skipped, file exists: {}".format(a.deseq_fix_xls)
        )
    else:
        try:
            run_shell_cmd(cmd)
