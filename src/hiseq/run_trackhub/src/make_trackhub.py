...
def get_args():
    ...
    parser.add_argument(
        "--user", default="Ming Wang", help="Who maintain the trackhub"
    )
    parser.add_argument(
        "--email", default="wangm08@hotmail.com", help="email for the hub"
    )
    parser.add_argument(
        "--server",
        default="yulab_tracks",
    ...
    parser.add_argument(
        "--server-url",
        default="http://159.226.118.232/upload/ucsc_trackhub",
...
def hub_url_checker(url):
    ...
    if hubCheck is None:
        ...
        p1 = subprocess.run(
            shlex.split("%s %s" % (hubCheck, url)), stderr=subprocess.PIPE
        )
        if p1.stderr.decode() == "":
            return True
        else:
...
def file_strand(fn):
    ...
    if (
        fn_name.endswith("fwd")
        or fn_name.endswith("watson")
        or fn_name.endswith("plus")
    ):
        strand = "f"
    elif (
        fn_name.endswith("rev")
        or fn_name.endswith("crick")
        or fn_name.endswith("minus")
    ):
        strand = "r"
    else:
        strand = "other"
...
def create_composite(
    trackdb, short_label, bigwig_files, bigbed_files=None, bedgraph_files=None
):
...
def main():
    ...
    if source_checker(args.source) is True:
        ...
        bw_dirs = [
            d for d in glob.glob(args.source + "/*") if os.path.isdir(d)
        ]
    ...
    trackhub.upload.upload_hub(
        hub=hub, host="localhost", remote_dir=server_path
    )
    ...
    hub_txt_url = os.path.join(
        args.server_url, args.server, hub_name, hub_name + ".hub.txt"
    )
