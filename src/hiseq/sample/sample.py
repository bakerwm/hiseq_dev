...
class Sample(object):
    ...
    def init_args(self):
        ...
        if isinstance(self.fx, str):
            ...
            raise ValueError(
                "-i expect str or list, got {}".format(type(self.fx).__name__)
            )
        # number
        if isinstance(self.number, int):
            ...
            raise ValueError(
                "-n expect int, got {}".format(type(self.number).__name__)
            )
        # output
    ...
    def sampleR1(self, fx):
        ...
        if file_exists(fx_out) and not self.overwrite:
            ...
            if self.random:
                log.info(
                    "extract {} random records from: {}".format(
                        self.number, fx
                    )
                )
...
def get_args():
    ...
    parser.add_argument(
        "-i", "--fx", nargs="+", required=True, help="fastx files"
    )
    parser.add_argument(
        "-o",
        "--out-dir",
    ...
    parser.add_argument(
        "-r", "--random", action="store_true", help="Get random subset records"
    ),
    parser.add_argument(
        "-O",
        "--overwrite",
