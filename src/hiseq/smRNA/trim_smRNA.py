...
Attention
=========
This script was designed for small RNA sequencing reads, by paired-end mode with 150bp read length.
...
class TrimSmRNA(object):
    ...
    def init_args(self):
        ...
        self.init_files()
        self.rep_list = [
            os.path.join(self.outdir, i)
            for i in fx_name(self.fq1, fix_pe=True)
        ]
    ...
    def init_files(self):
        ...
        default_files = {
            ...
            "trim_json": os.path.join(
                self.project_dir, self.prefix + ".trim.json"
            ),
...
def get_args():
    parser = argparse.ArgumentParser(description="trim_smRNA")
    parser.add_argument(
        "-1", "--fq1", nargs="+", required=True, help="read1 of FASTQ files"
    )
    parser.add_argument(
        "-2",
        "--fq2",
        ...
        required=False,
        default=None,
        help="read2 of FASTQ files, optional",
    )
    parser.add_argument(
        "-o",
        "--outdir",
