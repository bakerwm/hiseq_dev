...
class Cutadapt(object):
    ...
    def guess_adapter(self):
        ...
        if self.library_type is None and self.adapter3 is None:
            ...
            try:
                ...
                df = pd.DataFrame.from_dict(d, orient="index").sort_values(
                    0, ascending=False
                )
    ...
    def slice_adapter(self, x, recursive=False, **kwargs):
        ...
        if isinstance(x, str):
            ...
            out = [
                j
                for i in x
                for j in self.slice_adapter(i, recursive, **kwargs)
            ]
        else:
            out = []
        # filter ACTGN
        return [
            i for i in out if re.match("^[ACGTN]+$", i, re.IGNORECASE)
        ]  # list

    def prep_adapter(self):
        # to list; None => []
        # for 3' adapter
        self.adapter3 = self.slice_adapter(
            self.adapter3, self.recursive, direction=1
        )
        self.Adapter3 = self.slice_adapter(
            self.Adapter3, self.recursive, direction=1
        )
        # for 5' adapter
        self.adapter5 = self.slice_adapter(
            self.adapter5, self.recursive, direction=-1
        )
        self.Adapter5 = self.slice_adapter(
            self.Adapter5, self.recursive, direction=-1
        )
    ...
    def run(self):
        ...
        if j and not self.overwrite:
            log.info(
                "Cutadapt() skipped, file exists: {}".format(self.out_dir)
            )
        else:
            try:
                run_shell_cmd(cmd)
...
def get_args_io():
    ...
    parser.add_argument(
        "-1", "--fq1", required=True, help="FASTQ file, support (*.gz)"
    )
    parser.add_argument(
        "-2",
        "--fq2",
...
def get_args_trim(parser):
    parser.add_argument(
        ...
        default=None,
        type=str,
        choices=["TruSeq", "Nextera", "smallRNA"],
    ...
    parser.add_argument(
        ...
        default=15,
        type=int,
        help="Minimum length of reads after trimming, default [15]",
    )
    parser.add_argument(
        "-M",
        "--len-max",
        ...
        default=0,
        type=int,
        help="Maximum length of reads after trimming, default [0], ignore",
    )
    parser.add_argument(
        "--rm-untrimmed",
        action="store_true",
    ...
    parser.add_argument(
        "-g", "--adapter5", default=None, help="5-Adapter, default: None"
    )
    parser.add_argument(
        "-A",
        "--Adapter3",
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="overwrite exists files"
    )
    return parser
