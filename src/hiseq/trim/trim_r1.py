...
from hiseq.utils.file import (
    check_dir, file_exists, file_abspath, copy_file, symlink_file, remove_file
)
...
class TrimR1(object):
    ...
    def get_msg(self):
        msg = '\n'.join([
            ...
            '{:>20s} : {}'.format('remove untrimmed', self.discard_untrimmed),
            '-'*80,
        ])
        return msg
...
class TrimR1Config(object):
    ...
    def init_files(self):
        ...
        default_files = {
            ...
            'rm_dup_fq': os.path.join(self.rm_dup_dir, self.smp_name + '.fq.gz'),
            'rm_dup_fq1': os.path.join(self.rm_dup_dir, self.smp_name + '_1.fq.gz'),
            'rm_dup_fq2': os.path.join(self.rm_dup_dir, self.smp_name + '_2.fq.gz'),
            'cut2_fq': os.path.join(self.cut2_dir, self.smp_name + '.fq.gz'),
            'cut2_fq1': os.path.join(self.cut2_dir, self.smp_name + '_1.fq.gz'),
            'cut2_fq2': os.path.join(self.cut2_dir, self.smp_name + '_2.fq.gz'),
            'clean_fq': os.path.join(self.project_dir, self.smp_name + '.fq.gz'),
            'clean_fq1': os.path.join(self.project_dir, self.smp_name + '_1.fq.gz'),
            'clean_fq2': os.path.join(self.project_dir, self.smp_name + '_2.fq.gz'),
            'trim_stat': os.path.join(self.project_dir, self.smp_name + '.trim.stat'),
            'trim_json': os.path.join(self.project_dir, self.smp_name + '.trim.json'),
            'trim_yaml': os.path.join(self.project_dir, self.smp_name + '.trim.yaml'),
        }
        self = update_obj(self, default_files, force=True) # key
        check_dir([
            self.config_dir, self.cutadapt_dir, self.rm_dup_dir,  self.cut2_dir,
            self.log_dir,
        ])
...
class TrimR1se(object):
        ...
        if self.cut2_skipped:
            ...
            try:
                with xopen(fq_out, 'wt') as w, xopen(fq_in) as r:
                    min_len = sum([self.len_min, self.cut2_l, abs(self.cut2_r)])
    ...
    def wrap(self, n_total, n_cut1, n_rm_dup, n_cut2):
        ...
        header = ['#name', 'total', 'too_short', 'dup', 'too_short2',
                  'clean', 'percent']
        s = [self.smp_name, n_total, n_cut1_rm, n_rm_dup_rm, n_cut2_rm, n_cut2,
            n_clean_pct]
        s = list(map(str, s))
        msg = '\t'.join(header) + '\n' + '\t'.join(s)
...
class TrimR1pe(object):
    ...
    def cut2(self, fq1_in, fq2_in, fq1_out, fq2_out):
        ...
        if self.cut2_skipped:
            ...
            try:
                with xopen(fq1_out, 'wt') as w1, xopen(fq2_out, 'wt') as w2, xopen(fq1_in) as r1, xopen(fq2_in) as r2:
    ...
    def run(self):
        ...
        n_cut2_rm = self.cut2(self.rm_dup_fq1, self.rm_dup_fq2,
                              self.cut2_fq1, self.cut2_fq2)
        n_cut2 = n_rm_dup - n_cut2_rm
        # 4. wrap files
        self.wrap(n_total, n_cut1, n_rm_dup, n_cut2)
        # 5. remove temp files
        del_list = [cut1_fq1, cut1_fq2, self.rm_dup_fq1, self.rm_dup_fq2,
                    self.cut2_fq1, self.cut2_fq2]
        if not self.keep_tmp:
            remove_file(del_list, ask=False)
...
def get_args_i():
    ...
    parser.add_argument('-1', '--fq1', dest='fq1', required=True,
        help='reads in FASTQ files, support (*.gz), 1 file.')
    parser.add_argument('-2', '--fq2', dest='fq2', default=None,
...
def get_args_o(parser):
    ...
    parser.add_argument('--rm-dup', dest='rm_dup', action='store_true',
        help='remove duplicates')
    parser.add_argument('--cut2', '--cut-after-trim', dest='cut_after_trim',
        default='0',
        help='cut after trimming; example: 10: cut 10 from left; 0,-6; cut 6 from right; 9,-9, cut 9 from both ends, default [0]')
    parser.add_argument("--save-tmp", dest='keep_tmp', action="store_true",
