...
class Bam(object):
    ...
    def sort(self, outfile=None, by_name=False, overwrite=False):
        ...
        if os.path.exists(outfile) and overwrite is False:
            ...
            if by_name:
                tmp = pysam.sort(
                    "-@", str(self.threads), "-n", "-o", outfile, self.bam
                )
            else:
                tmp = pysam.sort(
                    "-@", str(self.threads), "-o", outfile, self.bam
                )
        return outfile
    ...
    def estimateInsertSizeDistribution(
        self,
        topn=10000,
        n=10,
        method="picard",
        ...
        assert self.is_paired(self.bam), (
            "can only estimate insert size from" "paired bam files"
        )
        samfile = pysam.AlignmentFile(self.bam)
        ...
        def get_core_distribution(inserts, n):
            ...
            return inserts[
                np.logical_and(
                    inserts >= threshold_min, inserts <= threshold_max
                )
            ]
        ...
        if method == "picard":
            ...
            inserts = np.array(
                [
                    ...
                    for read in samfile.head(n=topn)
            ...
            while iteration < max_chunks:
                inserts = np.array(
                    [
                        read.template_length
                        for read in samfile.head(
                            n=topn, multiple_iterators=False
                        )
                        if read.is_proper_pair
                        and not read.is_unmapped
                        and not read.mate_is_unmapped
    ...
    def estimateTagSize(self, topn=10, multiple="error"):
        ...
        if mi != ma:
            if multiple == "error":
                raise ValueError(
                    "multiple tag sizes in %s: %s" % (self.bam, sizes)
                )
            elif multiple == "mean":
...
class Bam2cor(object):
    ...
    def init_files(self):
        default_files = {
            ...
            "plot_cor_heatmap_png": os.path.join(
                self.out_dir, self.prefix + ".cor_heatmap.png"
            ),
            "log_heatmap": os.path.join(
                self.out_dir, self.prefix + ".cor_heatmap.log"
            ),
            "cor_counts": os.path.join(
                self.out_dir, self.prefix + ".cor_counts.tab"
            ),
            "cor_matrix": os.path.join(
                self.out_dir, self.prefix + ".cor.matrix"
            ),
            "plot_cor_pca_png": os.path.join(
                self.out_dir, self.prefix + ".cor_PCA.png"
            ),
            "log_pca": os.path.join(
                self.out_dir, self.prefix + ".cor_PCA.log"
            ),
        }
    ...
    def bam_summary(self):
        cmd = " ".join(
            [
                "{} bins --binSize {}".format(
                    self.multibamsummary, self.binsize
                ),
        ...
        if not os.path.exists(self.bam_npz):
            log.error(
                "Bam2cor() failed, output file not found: {}".format(
                    self.bam_npz
                )
            )
    ...
    def plot_cor_heatmap(self):
        ...
        if os.path.exists(self.plot_cor_heatmap_png) and not self.overwrite:
            log.warning(
                "Bam2cor.plot_cor_heatmap() skipped, file exists: {}".format(
    ...
    def plot_cor_pca(self):
        ...
        if os.path.exists(self.plot_cor_pca_png) and not self.overwrite:
            log.warning(
                "Bam2cor.plot_cor_pca() skipped, file exists: {}".format(
                    self.bam_npz
                )
            )
        else:
...
class Bam2fingerprint(object):
    ...
    def init_files(self):
        if not (
            ...
            and len(self.labels) == len(self.bam_list)
        ):
    ...
    def run_fingerprint(self):
        ...
        if file_exists(self.fp_png) and not self.overwrite:
            log.info(
                "Bam2fingerprint() skipped, file exists: {}".format(
                    self.fp_png
                )
            )
        else:
            try:
                ...
                log.error(
                    "Bam2fingerprint() failed, see {}".format(self.out_dir)
                )
    ...
    def run(self):
        msg = "\n".join(
            [
                "-" * 80,
                "{:>14} : {}".format(
                    "program", "hiseq.utils.bam.Bam2fingerprint()"
                ),
