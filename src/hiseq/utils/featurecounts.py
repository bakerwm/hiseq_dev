...
def read_fc_summary(x):
    ...
    try:
        ...
        if assign_min < 0.50:
            log.warning(
                "Caution: -s {}, {:.2f}% assigned, see {}".format(
                    s, assign_min, x
                )
            )
        log.info(assign_df)
...
class FeatureCounts(object):
    ...
    def init_args(self):
        ...
        if not self.stranded in [0, 1, 2]:
            raise ValueError(
                "stranded=, not valid, expect [0, 1, 2], \
                got {}".format(
                    self.stranded
                )
            )
    ...
    def check_files(self):
        ...
        bk = "\n".join(
            [
                "{:>14} : {}".format(
                    "pass" if file_exists(i) else "failed", file_prefix(i)
                )
        ...
        f = [file_exists(self.gtf), all(file_exists(self.bam_list))]
        if not all(f):
            print(msg)
    ...
    def get_args_gtf(self):
        ...
        if self.gtf_ext in [".gtf"]:
            ...
            raise ValueError(
                "unknown file, expect *.gtf, got {}".format(self.gtf)
            )
            s = ""
        return s
    ...
    def run_fc(self):
        ...
        if file_exists(self.count_txt) and not self.overwrite:
            log.info(
                "FeatureCounts() skipped, file exists: {}".format(
                    self.count_txt
                )
            )
        else:
            try:
                ...
                log.error(
                    "FeatureCounts() failed, see: {}".format(self.log_stderr)
                )
