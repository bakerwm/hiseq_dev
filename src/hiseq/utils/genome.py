...
class SetupGenome(object):
    ...
    def get_fa(self, genome):
        ...
        if is_supported(genome, key="genome"):
            fa1 = os.path.join(
                self.genome_dir, genome, "bigZips", genome + ".fa"
            )
            fa2 = os.path.join(
                self.genome_dir, genome, "fasta", genome + ".fa"
            )
            out = (
                fa1 if file_exists(fa1) else fa2 if file_exists(fa2) else None
            )
        else:
            out = None
        return out
    ...
    def get_gene_anno(self, genome, fmt="bed", db="ensembl", rmsk=False):
        ...
        if is_supported(genome, key="genome"):
            ad = os.path.join(
                self.genome_dir, genome, "annotation_and_repeats"
            )
    ...
    def get_piRC(self, genome, fmt="gtf"):
        ...
        if is_supported(genome, key="genome"):
            ...
            pd = os.path.join(
                self.genome_dir, genome, genome + "_piRNA_clusters"
            )
    ...
    def get_blacklist(self, genome):
        ...
        anno_dir = getattr(self, "anno_dir", ad)
        fc = {
            k: os.path.join(anno_dir, "blacklist", os.path.splitext(v)[0])
            for k, v in fn.items()
        }
    ...
    def run(self):
        ...
        msg = "\n".join(
            [
                ...
                "{:>10} : {}".format("config", gf),
                "{:>10} : {}".format(
                    "status", "pass" if len(dg) > 0 else "failed"
                ),
                "#" * 80,
            ]
        )
