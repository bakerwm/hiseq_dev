...
from hiseq.utils.hiseq_utils import (
    list_hiseq_dir,
    list_hiseq_file,
    is_hiseq_dir,
)
    ...
    if isinstance(x, str):
        if os.path.isdir(x):
            out = [
                i
                for i in list_dir(
                    x, include_dirs=True, recursive=args["recursive"]
                )
                if os.path.isdir(i)
            ]
            out.append(x)
...
def get_args():
    ...
    parser.add_argument(
        "x", nargs="+", help="list of directories"
    )  # positional arg
    parser.add_argument(
        "-o", dest="out_json", required=True, help="save output in json file"
    )
    parser.add_argument(
        "-n",
        "--name",
    ...
    parser.add_argument(
        "-a", "--abspath", action="store_true", help="return the absolute path"
    )
    parser.add_argument(
        "-f",
        "--force",
        dest="overwrite",
        action="store_true",
        help="overwrite exists output json file",
    )
    return parser
