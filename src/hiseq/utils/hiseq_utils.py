...
def list_hiseq_dir(x, hiseq_type="auto"):
    ...
    if a.is_hiseq:
        ...
        if any(
            [
                a.hiseq_type.endswith(i)
                for i in ["r1", "rp", "merge", "alignment"]
            ]
        ):
            out = [x]
            ...
            if a.hiseq_type.startswith("atac_"):
                ...
                if any(
                    [
                        a.hiseq_type.startswith(i)
                        for i in ["cnr", "cnt", "chip"]
                    ]
                ):
                    kt = ["ip_dir", "input_dir"]
    ...
    def init_args(self):
        ...
        if isinstance(d, dict):
            ...
            if isinstance(a, str):
                ...
                self.is_hiseq_merge = a.endswith(
                    "_merge"
                )  # merge multiple dirs
        #             else:
    ...
    def parse_fq(self):
        ...
        for f in f_names:
            ...
            for i, j in zip(fq1, fq2):
                ...
                if iname1 == f:
                    iname2 = fx_name(
                        i, fix_pe=True, fix_rep=False, fix_unmap=True
                    )
                    df.update({iname2: [i, j]})
            # update
            d.update({f: df})
        return d
